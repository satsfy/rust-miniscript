        ...
        match (self, key_request) {
            ...
            (Self::XPrv(descriptor_xkey), KeyRequest::Pubkey(public_key)) => {
                let xpriv = descriptor_xkey
                    .xkey
                    .derive_xpriv(&descriptor_xkey.derivation_path)
            ...
            (Self::XPrv(descriptor_xkey), KeyRequest::Bip32(key_source)) => {
                ...
                let derivation_path = match descriptor_xkey.matches(key_source) {
...
mod tests {
    ...
    fn get_key_xpriv_child_depth_one() {
        ...
        let child = master.derive_xpriv(&[child_number][..]).unwrap();
    ...
    fn get_key_xpriv_with_path() {
        ...
        let child = master.derive_xpriv(&derivation_path).unwrap();
    ...
    fn get_key_xpriv_with_key_origin() {
        ...
        for test_case in &cases {
            ...
            let exp_private_key = xpriv.derive_xpriv(&exp_derivation_path).unwrap().to_priv();

            let (_, keymap) = Descriptor::parse_descriptor(test_case.descriptor).unwrap();
    ...
    fn get_key_xpriv_with_key_origin_and_non_matching_path() {
        ...
        let (_, keymap) = Descriptor::parse_descriptor(descriptor).unwrap();
