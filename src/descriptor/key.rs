...
impl DescriptorSecretKey {
    ...
    pub fn to_public(&self) -> Result<DescriptorPublicKey, DescriptorKeyParseError> {
        let pk = match self {
            Self::Single(prv) => DescriptorPublicKey::Single(prv.to_public()),
            Self::XPrv(xprv) => DescriptorPublicKey::XPub(xprv.to_public()?),
            Self::MultiXPrv(xprv) => DescriptorPublicKey::MultiXPub(xprv.to_public()?),
        };
